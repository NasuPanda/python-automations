...
from utils.exceptions import InitializerError, ProcessTooManyError, ReadOnlyError
...
class Process:
    ...
    @classmethod
    def find_process_by_name(cls, process_name: str) -> psutil.Process:
        ...
        if len(processes) > 1:
            raise ProcessTooManyError("Two or more processes were found. Only supported process with unique name.")
        if not processes:
            return
        return processes[0]
...
class ProcessesMonitor:
    def __init__(
        self,
        monitored_process_names: list = config.MONITORING_PROCESSES,
        cpu_usage_thresholds: list = config.CPU_USAGE_THRESHOLDS,
    ) -> None:
        """Initialize an instance.
        ...
        self.monitored_process_names: list = monitored_process_names
        self.monitored_processes: dict = {}
        self.cpu_thresholds: dict = {}
    ...
    def __process_instance(self, process_name: str):
    ...
    def __update_process_instance_if_needed(
        self, process_instance: Process, current_process: psutil.Process
    ) -> Process:
    ...
    def update_process_instance_and_has_process_been_executed(self):
        ...
        for process_name in self.monitored_process_names:
            ...
            if current_process:
                ...
                if process_instance:
                    updated_process_instance = self.__update_process_instance_if_needed(
    ...
    @classmethod
    def find_process_by_name(cls, process_name: str):
        """Find process by name.
