...
class DataBase:
    ...
    def select(
        ...
        Args:
            ...
            where_logical_operator  LOGICAL_OPERATOR, optional): WHERE につかう論理演算子。 デフォルトは "AND" 。

        Returns:
            list[dict[str, str | int]]: 取得したデータ(辞書形式)のリスト。
        """
        if where:
    ...
    def select_last(self, columns: tuple[str, ...]) -> dict[str, str | int]:
        """対象カラムの最後のレコードを取得。
    ...
    def insert(self, to_insert_values: dict[str, str | int]) -> None:
        """レコードをテーブルに挿入する。

        Args:
            to_insert_values (dict[str, str | int]): 挿入するデータ。
    ...
    def update(
        ...
        Args:
            to_update (dict[str, str | int]): 更新する値の辞書。
            where (dict[str, str  |  int] | None, optional): WHERE の条件。キーを対象カラム、値を検索対象値とした辞書で指定する。 デフォルトは None.
            NOTE: WHERE column = value にしか対応していない。
            where_logical_operator  LOGICAL_OPERATOR, optional): WHERE につかう論理演算子。 デフォルトは "AND" 。
        """
